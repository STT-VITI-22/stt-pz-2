...
describe('Test suite for testing lib.js', () => {
  describe('Testuvannya funkcii vidmimannya', () => {
    ...
    it('konvertuye v chislo ta vidiymaye', () => {
      expect(lib.subtraction('13', '7')).toEqual(6);
    });
  });
  ...
  describe('testuye funkciyu chiloyi chastini vid chisla', () => {
    ...
    it('rachuye vid vidyemnogo chisla', () => {
      expect(lib.getWholeNumberPart(-4.9)).toEqual(-5);
      expect(lib.getWholeNumberPart(-4.5)).toEqual(-5);
      expect(lib.getWholeNumberPart(-10)).toEqual(-10);
    });
  ...
  describe('testuye funkciyu getVowels', () => {
    ...
    it('poverne masiv z golosnimi z ryadka', () => {
      expect(lib.getVowels('Golodnyak')).toEqual(['o','o', 'a']);
    });

    it('poverne golosni v dvoh registrah', () => {
      expect(lib.getVowels('OIUyea')).toEqual(['O', 'I', 'U', 'e', 'a']);
    });

    it('propuskaye simvoli ta chisla', () => {
      expect(lib.getVowels('G0l0dyak')).toEqual(['a']);
    });
  });

  describe('test suite for function concatString', () => {
    it('should concatenate strings', () => {
      expect(lib.concatString(['Roman', 'Golodnyak'])).toEqual('RomanGolodnyak');
      expect(lib.concatString(['z', 'x', 'c'])).toEqual('zxc');
      expect(lib.concatString(['I', 'am', 'god'])).toEqual('Iamgod');
    });
  });

  describe('testuye funkciyu splitString', () => {
    it('dilit ryadok po vvedenomu znaku', () => {
      expect(lib.splitString('Roman Golodnyak', ' ')).toEqual(['Roman', 'Golodnyak']);
      expect(lib.splitString('I?am?god', '?')).toEqual(['I', 'am', 'god']);
  ...
  describe('Testuye funkciyu hasSubString', () => {
    it('poverne true yakscho pidstroku znaydeno', () => {
      expect(lib.hasSubString('I am god', 'am')).toBe(true);
    });

    it('poverne false yakscho ne mistit pidstroku', () => {
      expect(lib.hasSubString('I am god', 'as')).toBe(false);
    });

    it('poverne true yaksho pidtroka taka yar ryadok', () => {
      expect(lib.hasSubString('Golodnyak', 'Golodnyak')).toBe(true);
    });
  });
